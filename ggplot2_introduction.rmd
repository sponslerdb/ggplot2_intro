...
subtitle: "Making easy plots hard and hard plot easy"
...
This tutorial will introduce you to the R package `ggplot2`. In general, you will find that `ggplot2` makes easy plots hard and hard plots easy. I assure you that, in the long run, this tradeoff is worth it. Once you master the basic `ggplot2` workflow, R will a canvas
...
Now, let's take up the challenge of coloring points by year. We have to *map* the `year` to color. 
...
Sometimes grid lines can make a graph feel cluttered. These, as well as many other plot features, can be customized using the `theme()` function. In this cases, we assign the argument `panel.grid` to the function `element_blank()`, which --- as you might expect --- causes the gird lines to disappear.
...
## Exercise 1: Explore the function `geom_smooth()`, which adds fit lines to scatter plots. 


### 1.1 What is the default smoothing method that `geom_smooth()` uses? 


### 1.2 Try switching to a linear fit line using `method = "lm`. 
...
### 1.3 Remove the `color = year` line from the first line of the code, and add `aes(color = year)` to the `geom_point()` call. What effect does this have on the behavior of the fit lines?
